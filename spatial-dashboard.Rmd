...
osm_amenity_sf.df <- opq(bbox = "minnesota", timeout = 60) %>%
  add_osm_feature(key = "amenity", value = c("cinema", "casino", "theatre", "events_venue")) %>%
  osmdata_sf()
...
amenity_points.df <- osm_amenity_sf.df$osm_points %>%
  ...
  mutate(amenity = str_replace_all(amenity, "_", " ") %>% str_to_title() %>% as_factor()) %>%
...
icons <- awesomeIcons(
  icon = amenity_label.df %>% st_drop_geometry() %>% dplyr::select(icon) %>% pull(),
  iconColor = "black",
  library = "fa",
  markerColor = amenity_label.df %>% st_drop_geometry() %>% dplyr::select(color) %>% pull()
)

# Create the leaflet
leaflet(data = amenity_label.df) %>%
  addProviderTiles("CartoDB.Positron") %>%
  addAwesomeMarkers(popup = ~ map(text_label, HTML), label = ~ map(text_label, HTML), icon = icons) %>%
  addLegend(
    title    = "Venue Type",
...
mn_pub_trans.ggmap <- hennepin_base.gg +
  ...
  labs(title = "Percent of Population that uses Public Transport to go to 
       Work, Hennepin County, MN 2019 ACS ") +
  scale_fill_distiller("% use",
